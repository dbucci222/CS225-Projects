...
class AVLTree
{
  ...
  private:
	...
	// Helper function for checking balance, returns bool concerning balance
	bool isBalanced(Node*& subtree);

	// Node* findLastUnbalanced(Node*& subtree);

	enum RotationType {
      right, left, rightLeft, leftRight
    };

	RotationType determineRotation(Node*& subroot);

	int getBalance(Node*& subroot);

	int updateHeights(Node*& subtree);
