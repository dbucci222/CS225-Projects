...
class BinaryTree
{
    public:
		 ...
		 * 
		 * @param treeNode a pointer to the treeNode passed in
		 * 
		 * @return a boolean telling if the tree is in order
		 */
		bool isOrderedRecursive(Node* treeNode) const;

		bool isOrderedRecursiveRight(Node* treeNode, T thisShouldBeLess) const;

		bool isOrderedRecursiveLeft(Node* treeNode, T thisShouldBeGreater) const;
